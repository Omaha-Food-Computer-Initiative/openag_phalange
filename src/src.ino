/*
  ...
  Modified for OpenAg May 21st, 2016
*/
...
#include <SoftwareSerial.h>
  ...
  #include <cban_config.cpp>
#else
  #include <utility/SerialFirmata.h>
#endif

// Common Bus Addressable Node
#include <cban_handler.h>

CbanHandler cban_handler;
#define CBAN 0x68
...
void sysexCallback(byte command, byte argc, byte *argv)
{
  ...
  String message, id, key, value, reply;

  switch (command) {
    case CBAN:
      // Reset strings
      message = "", id = "", key = "", value = "", reply = "";
      ...
      if (argv[0]) {
        // Process id
        int start_index = 0;
        int end_index = message.indexOf(',');
        id = message.substring(start_index, end_index);

        // Process key
        start_index = end_index + 1;
        end_index = message.indexOf(',', start_index);
        key = message.substring(start_index, end_index);

        // Send to get
        reply = cban_handler.get(id, key);
      }
      // Handle set
      else {
        // Process id
        ...
        // Process value
        start_index = end_index + 1;
        value = message.substring(start_index);

        // Send to set
        reply = cban_handler.set(key, id, value);
      }
      ...
      if (reply != "") {
        ...
        Firmata.write(CBAN);
          for(int i=0; i<reply.length(); i++) {
            Firmata.write((int)reply.charAt(i));
          }
        Firmata.write(END_SYSEX);
      }
      break;
